...
test('test parser', () => {
  ...
  expect(result[2]).toEqual({ light: 'Lounge lamp', state: 'off', transition: 0 });
  ...
  result = parser.parse('{<Living room> any} [Lounge lamp] off transition 30');
  expect(result[0][1][0]).toEqual({ "room": "Living room", "state": "any" });
  expect(result[2]).toEqual({ light: 'Lounge lamp', state: 'off', transition: 30 });
  expect(result[0][1][1].length).toEqual(0);
  ...
  expect(result[2]).toEqual({ light: 'Kitchen night light', state: 'on', transition: 0 });
});
